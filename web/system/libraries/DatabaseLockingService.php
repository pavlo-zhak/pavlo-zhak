...
class DatabaseLockingService {
    ...
    public function isLocked(string $lockName): bool
    {
        if ( ! $this->waitForLock)
        {
            $result = App::get_ci()->s->sql('SELECT IS_FREE_LOCK("' . $lockName . '") as `lock`')->one();
    ...
    {
        $lock = App::get_ci()->s->sql('SELECT GET_LOCK("' . $lockName . '", ' . $this->timeout . ')  as `lock`')->one();
    ...
    {
        $lock = App::get_ci()->s->sql('SELECT RELEASE_LOCK("' . $lockName . '")  as `lock`')->one();
        if ( ! empty($lock) && $lock['lock'] == 1)
        {
            return TRUE;
        } else
